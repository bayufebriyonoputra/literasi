...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Modul &nbsp;</span>Laporan Unggah Karya</h4>
                ...
                <tbody>
                    @foreach ($karya as $k)
                        <tr>
                            ...
                            <td>{{ $k->karya->karya }}</td>
                            <td><a href="{{ asset('uploads/' . $k->file_karya) }}" class="btn btn-info">Lihat File</a></td>
                            <td>{{ $k->status_kurasi }}</td>
                            <td>{{ $k->review ? $k->review : '-' }}</td>
                            <td>
                                <a href="/unggah_karya/{{ $k->id }}/edit" class="btn btn-warning"><i
                                        class="fa-solid fa-pen-to-square"></i></a>

                                <form action="/unggah_karya/{{ $k->id }}" class="d-inline" method="POST">
                                    @csrf
                                    @method('DELETE')
