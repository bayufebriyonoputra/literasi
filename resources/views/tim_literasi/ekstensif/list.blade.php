...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <p class="text-primary fs-5">Catatan : kriteria ketercapaian pembiasaan ekstensif adalah jika murid mampu membaca
            ...
            buku</p>
        <p>Pilih Kelas</p>
        <form method="POST" action="/filter-ekstensif" class="mb-4">
            @csrf
            <div class="row">
                <div class="col-md-10">
                    ...
                    <div class="col-md-4">
                        <select name="tahun_pelajaran" id="tahun_pelajaran" class="form-select">
                            ...
                            @foreach ($list_tapel as $tapel)
                                <option value="{{ $tapel }}" {{ $tapel == $selected_tapel ? 'selected' : '' }}>{{ $tapel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
            ...
            <table class="table" id="table">
                ...
                <tbody>
                    @php
                        $i = 1;
                    @endphp
                    @foreach ($ekstensif as $a)
                        @foreach ($a as $e)
                            <tr>
                                <td>{{ $i }}</td>
                                <td>{{ $e[0]->siswa->nama }}</td>
                                <td>{{ $a->count() }}</td>
                                <td>{{ $a->count() >= 3 ? 'Memenuhi' : 'Belum Memenuhi' }}</td>
                                <td><a href="/inovasi-detail-ekstensif/{{ $e[0]->siswa_id }}"
